	...
	//hub.run(1000/100);
       ...
       // collector.print();
		...
		if (1)//duration > 1)
		{
			...
			switch (myo::Pose::fist)    //(collector.currentPose.type())
			{
			case myo::Pose::fist:
				std::cout << "sending fist: " << std::endl;
				arduino->writeSerialPort(poseBuffer1, 27);
			...
			case myo::Pose::doubleTap:
				std::cout << "sending Double Tap: " << std::endl;
				arduino->writeSerialPort(poseBuffer2, 27);
				readResult = arduino->readSerialPort(incomingData, MAX_DATA_LENGTH);
				printf("%s", incomingData);
				break;
			case myo::Pose::fingersSpread:
				std::cout << "sending Fingers Spread: " << std::endl;
				arduino->writeSerialPort(poseBuffer3, 27);
				readResult = arduino->readSerialPort(incomingData, MAX_DATA_LENGTH);
				printf("%s", incomingData);
				break;
			case myo::Pose::waveIn:
				std::cout << "sending Wave In: " << std::endl;
				arduino->writeSerialPort(poseBuffer4, 27);
				readResult = arduino->readSerialPort(incomingData, MAX_DATA_LENGTH);
				printf("%s", incomingData);
				break;
			case myo::Pose::waveOut:
				arduino->writeSerialPort(poseBuffer5, 27);
				readResult = arduino->readSerialPort(incomingData, MAX_DATA_LENGTH);
				printf("%s", incomingData);
				break;
			}
